...
class ElfFileSystem
  ...
  def read_command(curr_cmd)
    case curr_cmd.strip
    when /\Acd .*/
      change_dir curr_cmd.split[1]
    when 'ls'
      puts @current_directory.ls
    end
  end

  def reset_dir!
    @current_directory = @dir_path.first
    @dir_path = [@dir_path.first]
  end

  def curr_dir_path
    return '' if @dir_path.count == 1

    "#{@dir_path[1..].map { |d| d.name.to_s }.join('/')}/"
  end
